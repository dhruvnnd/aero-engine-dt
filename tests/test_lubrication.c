...
static void test_relief_valve_caps_pressure(void) {
  LubeConfig c = lube_config_default();
  LubeState s;
  lube_state_init(&s);
  lube_step(&s, &c, 6000.0, 15.0); /* absurd rpm, cold thick oil */
  CHECK_NEAR(s.oil_press_kpa, c.relief_valve_kpa, 1e-9);
}
...
static void test_model_oil_pressure_is_live(void) {
  ...
  CHECK(st.lube.oil_press_kpa <= sync.lube_config.relief_valve_kpa + 1e-6);
}
...
static const TestCase CASES[] = {
    ...
    {"lube.relief_valve_caps_pressure", test_relief_valve_caps_pressure},
